...
//		counter = 6;
	...
	public void allPrint() {
		// TODO Auto-generated method stub
		for (HumanDto humanDto : human) {
			if (humanDto != null && !humanDto.getName().equals("")) {
				System.out.println(humanDto.info());
			}
		}
	}
	...
	public void pitchSort() {
		...
		for (int i = 0; i < humanB.length - 1; i++) {
			for (int j = i + 1; j < humanB.length; j++) {
				if (humanB[i] != null && !humanB[i].getName().equals("") 
						&& humanB[j] != null && !humanB[j].getName().equals("")) {
					PitcherDto b1 = (PitcherDto)humanB[i];
					PitcherDto b2 = (PitcherDto)humanB[j];
		...
		for (int i = 0; i < humanB.length; i++) {
			if(humanB[i] != null) {
				System.out.println((i + 1) + "위 "
						+ "이름:" + humanB[i].getName()
						+ " 방어율:" + ((PitcherDto)humanB[i]).getDefence());
			}
		}
	}
